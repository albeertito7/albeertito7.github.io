...
(function () {
  var header = $('.js-nav'),
      burger = header.find('.js-nav-burger'),
      wrapper = header.find('.js-nav-wrapper'),
      html = $('html'),
      body = $('body');
...
/*var audioElement = false,
  audios = ["libertad.mp3", "sale-el-sol.mp3", "bailando.mp3"],
  audio = undefined;
...
$('#sound a').click(function(e) {
  ...
  if($(this).parent().hasClass('activated')){
      ...
      $(".container-dancing").hide();
      $(".main__view").show();
  }else {
      $(this).parent().addClass('activated');

      $(".main__view").hide();
      $(".container-dancing").show();

      if(!audioElement) {
          
          audio = Math.floor(Math.random()*audios.length);
          audioElement = new Audio('/assets/audio/' +  audios[audio]);

          audioElement.addEventListener('ended', function(e) {
              new Promise(r => setTimeout(r, 2000));
              audio = (audio + 1) % audios.length;
              audioElement = new Audio('/assets/audio/' + audios[audio]);
              audioElement.play();
         });
      }
...
/*particlesJS.load('particles-js', '/assets/js/plugins/particles/particles.json', function() {
...
$(document).ready(function() {
  ...
  tl.from(".nav a, .nav li > div", { duration: 1.5, opacity: 0, scale: 0.1, ease: "back" });
  //gsap.to(".nav-logo", { duration: 2, x: 300, backgroundColor: "#560563", borderRadius: "20%", border: "5px solid white", ease: "elastic" });
});